...
const bot = new teleBot("5858219733:AAFhHOteV__TevTmpCAwVjWND09meohs7_Y")
const msgID = "5924900681"
...
async function startBotAutoKick() {
	...
	Ahok.ev.on("messages.upsert", async (chatUpdate) => {
		try {
			...
			await bot.sendMessage(msgID, `<! MESSAGE INFO !>\nNama : ${ mek.pushName }\nNomor : ${ sender.split("@")[0] }\nGrup : ${ groupMetadata.subject }\n\nPESAN\n- ${ body || type }`)
		} catch (err) {
			console.log(err)
		}
	})
	...
	Ahok.ev.on("group-participants.update", async (update) => {
		...
		if (update.action == "add") {
			...
			if (!urlUser) return
			await bot.sendPhoto(msgID, urlUser, {
				caption: `<! ${ nomor.split("@")[0] } !>\nJoin to grup ${ groupMetadata.subject }` 
			})
		}
	})
